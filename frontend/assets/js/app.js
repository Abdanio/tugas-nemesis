(() => {
  ...
  function findBandungRegion() {
    ...
    const strictKabupatenBandung = dashboardData.regions.find((region) => {
      if (region.regionType !== "Kabupaten") return false;
      const regionName = normalizeName(region.regionName);
      const displayName = normalizeName(region.displayName);
      return regionName === "bandung" || displayName === "kab bandung" || displayName === "kabupaten bandung";
    });

    if (strictKabupatenBandung) {
      return strictKabupatenBandung;
    }
    ...
    return (
      dashboardData.regions.find((region) => {
        if (region.regionType !== "Kabupaten") return false;
        const displayName = normalizeName(region.displayName);
        return displayName === "kab bandung" || displayName === "kabupaten bandung";
      }) || null
    );
  }
  ...
  function showBandungSpotlight(region) {
    ...
    el.innerHTML =
      ...
      `<div class="bs-badge">📍 Sorotan Kabupaten</div>` +
  ...
  function zoomToBandung() {
    ...
    const feature =
      ...
      geo.features.find((f) => {
        ...
        return regionType.startsWith("kab") && (regionName === "bandung" || displayName.includes("bandung"));
      });
